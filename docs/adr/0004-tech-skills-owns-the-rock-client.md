...
- **rock-tools stays canonical, with `just sync-rock` plus a CI check that the copies match.** Keeps Rock development private and makes publishing an explicit act. Rejected because the private dev loop was protecting nothing that is actually private, and the CI job would exist solely to detect a problem we can decline to create.
